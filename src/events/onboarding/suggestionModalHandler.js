/**
 * -----------------------------------------------------------
 * SmokeLog - Suggestion/Enhancement Modal Handler
 * -----------------------------------------------------------
 *
 * Description: Handles suggestion/enhancement submissions and creates
  ...
  async execute(interaction) {
    ...
    if (interaction.isButton() && validTypes[customId]) {
      ...
      const modal = new ModalBuilder()
        .setCustomId(`submitSuggestionForm:${customId}`)
        .setTitle(`${typeLabel} Submission`);

      const subject = new TextInputBuilder()
        .setCustomId("subject")
        .setLabel("Title or Subject")
    ...
    if (
      ...
      interaction.customId.startsWith("submitSuggestionForm:")
    ) {
      ...
      const sanitizedName = user.username
        .toLowerCase()
        .replace(/[^a-z0-9]/g, "");
      const channelName = `suggest-${typeLabel.toLowerCase()}-${sanitizedName}-${suffix}`;

      const category = guild.channels.cache.get(
        config.CHANNELS.SUGGESTION_SUBMISSION_CATEGORY
      );
      ...
      if (
        ...
        logger.warning(
          `Missing ManageChannels permission for suggestion category (${config.CHANNELS.SUGGESTION_SUBMISSION_CATEGORY})`
        );
        return await interaction.reply({
          content:
            "❌ I don't have permission to create a suggestion ticket. Please contact staff.",
          flags: 64,
        });
      }
      ...
      const embed = new EmbedBuilder()
        .setTitle(`💡 New ${typeLabel} Submitted`)
        .addFields(
      ...
      await interaction.reply({
        content: `✅ Your ${typeLabel.toLowerCase()} has been submitted: <#${
          ticketChannel.id
        }>`,
        flags: 64,
      });
    ...
    if (interaction.isButton() && interaction.customId === "closeTicket") {
      ...
      try {
        ...
        await channel.send(
          "⏳ This suggestion ticket will close in 30 seconds..."
        );
